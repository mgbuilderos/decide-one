...
| ~~O9~~ | Search Console verification | founder | — | **closed 12 Sep** — verified as a Domain property via Cloudflare's domain-name-provider integration, owner `maulik.payment@gmail.com` (deliberate). TXT `google-site-verification=f3Cw_TF3C-LEZLF_9vCAMBloZn-wE1KEue1bjfwqvZQ` confirmed on the authoritative nameserver. Googlebot fetches `sitemap.xml` 200 and `robots.txt` 200 |
