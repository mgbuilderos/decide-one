...
### 2026-09-12 01:42 — unattributed

Front-end forensic pass. Four faults, four root causes. (1) The gold D1: the initial commit's icon was a gold-embossed monogram (#D4AF37), replaced by the monochrome mark in 0c0b030 — but sw.js pre-cached /icon.svg into CACHE_NAME 'decideone-priority-v1.0.4', which never changed, and activate only evicts caches whose name differs. Visitors were served the first commit's icon for weeks with no way for the site to correct it. sw.js rewritten: cache-first only for /assets/* (content-hashed, already marked immutable in _headers), network-first for everything else, cache name bumped to evict the poisoned store. (2) The blogs were unreachable: href="/guides" appeared zero times in the React app. The 7 links added to index.html's #root are destroyed by createRoot on mount, so only non-JS crawlers ever saw them. Added a Writing section with six real links, and guide links to the nav and footer. (3) The stale text was the same #root prerender painting as unstyled black-on-white prose before the bundle arrived — there was no critical CSS in the head at all. Added it, matching the values the bundle sets on body so the repaint changes no colour. (4) The marker poking out of the 3D journal was the spine label at y=2.08, z=0.075 — behind the page planes, so the only visible part was the 0.19 protruding above the cover. No position fixed it; removed per VISION §13. Entry point now splits on who: a written day in storage opens the journal directly, a first visit gets the front door. Landing cut from ten sections to six — craft, perspectives, devices and the FAQ (which duplicated /faq/) are gone.

---

### 2026-09-12 01:45 — unattributed

The landing page's 3D scene fell back to decideone-studio-v2.webp whenever WebGL was still loading or unavailable, and that render carries the PRIMACY label on the notebook — the pre-rename brand, live on the home page. The renamed asset (decideone-studio-d1) already existed and was used by the outer Suspense fallback; only the inner one still pointed at v2. Repointed, and its alt text corrected — it described 'a woven label' that is no longer what the image shows. Rule 17 could not catch this because it reads source, not pixels. Deleted four orphaned renders: both v2 files, and the two pro-devices files the cut section used. Checked the other gold: .monogram-gold-foil in the notebook cover was already converted to black ink per VISION §12.1 and only the class name still says gold, so the persisted 'gold'/'blind' value is left alone rather than migrated for no user benefit.

---

### 2026-09-13 02:01 — unattributed

Architecture audit of the verification system, written after five live defects passed every gate. Finding: qc_audit.js is a source linter treated as a product-correctness gate — 865 lines, 26 rules, and it reads src/ and nothing else. check_content.js reads content/ and index.html. Nothing had ever read public/, worker/ (175 lines of deployed request handling that writes to D1) or dist/ (the artefact itself), and zero of 2,300 lines of verification render, rasterise or fetch anything. Four failure classes documented in ARCHITECTURE_AUDIT.md: (A) surfaces outside the gate, (B) existence checked instead of effect — now found three separate times, counting Rule 0, Rule 25 and Rule 18, which makes it structural rather than careless, (C) no orphan detection, (D) recorded knowledge that cannot be reached. D was the one that cost this session: the gold foil removal was recorded in full at DECISION_LOG.md line 845 while AGENTS.md says to read documentation only when brief tells you to and brief shows five entries. Built layers 1 to 5. scripts/qc_artifact.js runs after the build inside predeploy and checks references resolve, public/ has no orphans, the home page reaches every content section, the service worker cache name is build-derived, and the deployed Worker parses. build_content.js now stamps CACHE_NAME from a hash of the built assets, so the cache can never freeze again — derive, do not remember. npm run why searches the whole record and returns the gold foil decision in two seconds. Each of the five assertions was deliberately broken to prove it fires: two did not. The reference check read href and src but not content=, so a dead og:image would have shipped; the link-graph check counted index.html including the #root prerender that createRoot destroys, which reproduced the exact defect it was built to catch. On its first clean run the fixed gate found a live instance: /methods/ was in the sitemap and linked from nowhere on the home page. Also fixed log_session.js and handoff.js, which stamped a UTC date beside a local time — at 02:00 IST on the 13th that wrote 2026-09-12 02:00, so entries near midnight landed on the wrong day and the log disagreed with git.

---

### 2026-09-13 02:06 — unattributed

Regression I shipped, and the fix. The critical CSS added to index.html was scoped to #root. #root is an ID, so #root h2 has specificity 0,1,0,1 and outranks every class selector in the bundle: the final CTA's h2 was forced from 86.4px to 18px while keeping the landing page's -4px tracking, which collapsed 'Three lines. Then get to work.' into overlapping letters, and 'Everything, from today. Nothing held back.' the same way. #root a pinned every link to #18181B so the nav, brand name and footer went near-invisible on black and the light/dark toggle could not override them, and #root p, #root li put a 14px margin and 1.7 line-height on every paragraph and list item in the journal as well as the landing page — which is what broke the tool's zero-scroll. Fixed by scoping to #prerender, an id on the crawlable summary itself, which createRoot destroys on mount, so the rules can only ever apply to what they were written for. Verified: CTA back to 86.4px, nav back to rgb(150,150,150), and the daily view measures docScrollHeight equal to viewport height with zero overflowing children at 1440x900, 1366x768 and 390x844. Added check 7 to qc_artifact.js for the class, negative-tested. Note for whoever reviews Rule 4: it asserts App.jsx contains the strings overflow-hidden and h-screen and never tests whether the page scrolls, so it passed throughout while the tool scrolled. That is the existence-vs-effect class in ARCHITECTURE_AUDIT.md section 3, and it is now the fourth instance.

---

### 2026-09-13 02:40 — unattributed

Built the visual regression layer, the one ARCHITECTURE_AUDIT.md section 6 named as missing. Chrome driven over CDP with Node's built-in WebSocket, sharp for the pixel diff, no new dependency. 12 surfaces at desktop, laptop and phone, light and dark, each asserting: no console errors, no tracking tighter than -0.12em (the signature of the collapse I shipped yesterday), no text under 3:1 against its composited background, no overflow outside a deliberate scroller, no content out of reach on an instrument view, and pixels within 0.35 percent of a committed baseline. Negative-tested against the three real defects: the #root critical CSS caught as -0.144em tracking on 18px headings, an invisible link caught at 1.01:1, a layout shift caught as 1.4 to 7.7 percent pixel change. Two of my own checks were wrong first. The background reader took the first non-transparent ancestor so a 2.5 percent alpha black overlay on white paper read as pure black and invented contrast failures; backgrounds are composited now. Worse, the zero-scroll assertion could never fire: the app sets h-screen overflow-hidden so documentElement.scrollHeight is pinned to the viewport and content is clipped rather than scrolled, which is worse because it is unreachable. Proved dead by rendering the daily view in a 300px viewport and watching the check pass, then rewritten to measure inner containers for both scrolled and clipped content. That is Rule 4's defect exactly, and the fourth instance of existence-versus-effect. First honest run found 18 unreadable elements between 1.48:1 and 2.52:1, fixed by raising 96 small-text colours to neutral-500, plus four inherited cases fixed by hand. It also found the instrument pushing up to 97px of the day out of reach below 740px of viewport height. Two steps fixed 768 and above and took 720 from 77px to 25px; the rest needs the priority row height reduced, which is a design decision and is recorded in WORK_REMAINING.md with the measurements. Those heights are deliberately not in the gate's surface list: a known-red gate teaches every agent to ignore it. test:visual is wired into predeploy.

---

### 2026-09-13 03:32 — unattributed

No scroll, on either axis, anywhere in the instrument — the founder's instruction, made absolute. Measured across four views and eight viewports (1440x900, 1366x768, 1366x700, 1366x640, 1280x600, 390x844, 360x740, 320x568): all 32 combinations now clean, nothing scrolls and nothing is clipped. Three non-overlapping density tiers by viewport height: compact 761-820 steps the execution panel, notebook padding and clock (88 to 60px); tight at 760 and below drops the clock, takes priority rows 48 to 36px and the section header 36 to 28px, and tightens block gaps and notebook padding; tiny at 620 and below tightens the day-condition dialog while keeping both the label and what it does, because dropping the explanation would make the choice harder. A narrow-width step at 400px tightens the perspective buttons and method icons and takes the date header from 18px to 15px so the whole date fits at 320px instead of being cut mid-word with text-overflow clip. The hard lesson: arbitrary Tailwind media variants all carry the same specificity, so [@media(max-height:820px)] and [@media(max-height:760px)] both matching means source order decides, and Tailwind emitted the wider one last. Several density rules were dead while reading as correct in the source — found only by measuring a box at 120px whose single child was 20px. The tiers are written as non-overlapping ranges now. Fifth instance of existence-versus-effect, this time in the fix rather than the gate: a rule that is present is not a rule that is in effect. Also checked P11 before overriding it — it says nothing about scrolling, and grep -i scroll DECISIONS.md returns nothing, so the code comment claiming 'it scrolls (P11)' attributed a behaviour to a decision that does not mention it. No conflict. Rule 6 had to be corrected rather than weakened: it checked the literal string 'p-6 embossed-notebook', testing that two class names sat adjacent in source rather than that the padding was 24px. It now reads the notebook line and asserts p-6 is on it; negative-tested by removing p-6 entirely, which still fails. visual_check.js gained daily-small (1280x600), daily-tiny and weekly-tiny (320x568) plus a horizontal clipping check that fires only on overflow hidden without an ellipsis, since a figure with overflow-x auto is still reachable.

---

### 2026-09-14 14:50 — unattributed

Wrote UI_BRIEF.md: a self-contained brief for any agent doing interface work (ChatGPT Codex, Antigravity, Claude Code), with the kick-off prompt as Appendix A. It quotes the vision rather than paraphrasing it, states which documents are stale, defines frameworks as modes rather than skins, sets the design direction (premium means precise), says exactly what each gate does and does not catch, lists decisions already made (gold gone, black and white only, instrument not book — resolving BR8 toward the instrument register, subject to founder veto — no celebration), designs every state, and records nineteen learnings. Registered in brief.js and in AGENTS.md and CLAUDE.md, which replace the line telling agents to trust LANDING_PROTOTYPE.md; that file described a Primacy prototype with six methods and habits, and is rewritten to what is built. DECISIONS.md rows P8, C5, C6, BR4 and BR5 amended in place. qc_audit.js now fails if AGENTS.md and CLAUDE.md drift apart — two copies of one rulebook with nothing keeping them identical. Rule audit recorded in ARCHITECTURE_AUDIT.md section 7e. The Rule 4 rewrite had deleted appPath, which Rules 6 and 19 use; node --check passed and the audit crashed on every run until it was run unfiltered. Declared at module scope, and every changed rule broken on purpose and seen to fail. App fixes: number keys 1 to 4 now match the visible tabs (Weekly had no key); day-of-year counts calendar days, since Math.floor over milliseconds read Day 181 on day 182 in DST timezones; the morning prompt closes when the person leaves today's page instead of covering Weekly. Visual gate: time frozen at 09:30 IST on 14 September 2026 with the timezone pinned, because date text made every baseline fail the next day; baselines per platform; daily surfaces dismiss the prompt so the instrument itself is baselined, where until now every daily baseline was a picture of the dialog, plus a daily-prompt-tiny surface; 30-second CDP timeouts, progress lines and native-dialog handling so it cannot hang silently; the keyboard pass runs in its own tab and waits for each view, because a fixed sleep reported late views as dead keys. Nothing shares first is the decided tagline (N30), not a copy error. Defects visible in the new baselines, left for the redesign: 0/3 done with a red zero on an untouched day, a one-word widow in the execution hint, placeholders cut mid-word at 320px, and a 2.5-second switch back to Daily in headless Chrome.
...
### 2026-09-14 23:57 — unattributed

Step 5a (book chrome): retired the notebook cover and the twelve month illustrations. Deleted NotebookCover.jsx and src/data/monthIllustrations.jsx; removed showCover/coverAnimation state, the open/close/toggle handlers, the C shortcut, HeaderToolbar's View Cover entry and showCover props, the menu's Cover link, the book-open/book-close sounds (nothing else played them), the cover and monogram CSS, and the illustration block from ExecutivePrivacyOverlay (shutter otherwise unchanged). ?view=cover now opens daily via src/utils/viewParam.js. Rule 0 drops the illustrations from governedSurfaces and fails if a retired module is reached from main.jsx or back on disk; Rule 15 now fails on cover names in any reached file and executes viewFromParam('cover'). Nine negative tests run (T1-T9), all fired, T6 confirmed comments do not trip it. BR8, B-15 and C6 (line 257) amended in place; VISION 10.5/12.4/12.5, BRAND_BOOK 8, BRAND_HOUSE and UI_BRIEF 6.1/7.3/10 given dated notes. The ?giftFrom= greeting went with the cover: founder to confirm no gift link is in circulation. Side effect: removing the plain C branch un-shadows Cmd+Shift+C (closure) in the key handler.

---

### 2026-09-15 00:03 — unattributed

Removed the woven DECIDE ONE tag (book chrome retired by BR8/P11): the menu-opening button in App.jsx, both copies on the privacy shutter cards in ExecutivePrivacyOverlay.jsx, and its 69 lines of CSS in index.css. Dropped pt-5 sm:pt-6 from the <main> stage, which only made room for the tag (top:-24px); the snapped notebook height subtracts the stage padding, so it gains one 24px step where the viewport allows. The menu stays reachable from the header and the M key. Inverted Rule 10: any woven tag under src/, in markup or CSS, fails (comments stripped; message says whether main.jsx reaches the file). Matches 'woven', not 'DECIDE ONE', because the weekly PDF heading uses that label. Negative-tested: markup in the overlay, CSS in index.css, and an unreachable file each fail; a comment does not. Visual baselines will differ (tag gone, stage 20-24px taller); that is for the Visual stage.

---

### 2026-09-15 00:25 — unattributed

Flatten the sheet (critic step 5c), finishing a cut-off implementer's diff after verifying each hunk. embossed-notebook becomes instrument-sheet on the one sheet className in App.jsx: flat 1px hairline in index.css, no box-shadow, transition-all removed. Dead chassis CSS deleted once (real-3d-book-chassis, notebook-spine-crease, soft-frost-vellum, diary-spread-*, exploded-*, promotion-calibrated-elevation, gilded-fore-edge), each confirmed unused in src, index.html and scripts. .page-turn-edge-btn rewritten once: neutral greys, outline:none !important and its :focus rule removed so the global focus-visible ring draws focus, backdrop-filter dropped. Privacy shutter card loses shadow-2xl, embossed-notebook and paper-block-edge only; the partial diff had added instrument-sheet there, which I took out so the shutter change is removal only. Rules 6 and 19 rewritten on the shared SHEET_TOKEN anchor: Rule 6 needs exactly one sheet className with p-6; Rule 19 needs a reached hairline and fails any box-shadow, text-shadow, drop-shadow, gradient or 3D on the sheet (CSS, pseudo-elements included), any shadow, [box-shadow:] or transition-all utility on it, and any retired chassis class under src. 13 negative tests each failed with their message; a comment naming the chassis stays green. visual_check.js had no embossed-notebook selector, so it was not edited. Visual baselines will change (sheet border and shadow); the Visual stage re-records them.
...
### 2026-09-15 09:21 — codex

Restarted the interrupted design pass and preserved its layout work: neutral shared tokens and static-content styles; readable type and compact rows; short Top 3 placeholders, strict shared Ivy Lee locking, classification before Matrix writing; complete yearly calendar positions; empty owner defaults; accurate labelled idle clock, retained paused session, and closed-day wording. Reviewed the paused-state screenshot and recorded it with test:visual --update --surface=state-paused. Baselines: landing desktop/mobile now show the quick start; daily desktop/laptop/mobile/small/tiny and prompt-tiny use neutral readable compact styling; weekly desktop/tiny, monthly desktop and yearly desktop/tiny share it; guide desktop/mobile/dark and FAQ desktop match the tokens; new written/Ivy/running/overrun/paused/closed surfaces cover populated 320x568 states. Repaired Rules 3, 10, 18 and artifact Rule 26; every changed rule was deliberately broken and observed failing, including a renamed rendered brand badge in both themes. Build, QC, artifact and all 23 default Chrome surfaces plus keyboard routing pass. Full populated Matrix/Today/carry-forward no-scroll behavior still requires the founder decision on paging or scrolling; secondary/dialog and month-old-return coverage, final social capture and deploy remain. This is a recoverable checkpoint, not a finished release.

---

### 2026-09-15 09:35 — codex

Founder rejected the flat visual result and large square bullets and asked to find the earlier pre-domain local design. Located the 9 September Primacy design history in task Create 3D landing page prototype, surviving screenshot_daily_verified.png with the two-page spread, central fold, paper depth and round bullets, and earliest Git snapshot eaa00d7 (10 September, already renamed Decide One). Restored only its frontend into /private/tmp/decide-one-book-eaa00d7 and started an isolated review preview at http://127.0.0.1:5180/?view=daily, reusing existing dependency files with a separate Vite cache. Captured output/playwright/recovered-book-desktop.png. No rollback, merge, or product-source changes made; current 5173 preview and live deployment are unchanged. Current square checkbox CSS came from 4ee0ccc and was an implementation choice, not required by the supplied brief. Confirm the recovered visual reference with the founder before applying restoration against current feature behavior.
...
### 2026-09-16 13:48 — unattributed

Built support (B-46, VISION §11.1). src/utils/support.js: channels from VITE_SUPPORT_* (each shown only when set), suggested UPI amounts and GitHub Sponsors tiers as plain numbers, upi:// link builder, on-device India detection from time zone and languages, and the quiet ask (14 closed days, once a day, waits 14/30/90 days after Not now, Don't ask again ends it). SupportModal.jsx replaces the unreachable PatronUpgradeModal: UPI tab (four suggested amounts, another amount, QR code via lazily imported qrcode 1.5.4 MIT, deep link on phones, copyable UPI ID) and GitHub tab (three tiers, Sponsor, See the code, MIT line); neutral tokens only, Escape captured so it does not reach the privacy shutter. SupportNote.jsx is the non-modal quiet ask, 400ms after closing, never over the closing dialog. Menu gains Support Decide One. Rule 24 now executes support.js against VISION's canonical support line and checks the ask stays quiet; Rule 0 governs SupportModal and support.js. Negative tests: changed amount, earlier ask, twice a day, no longer back-off, rupee figure in src, VISION line removed, modal unreachable — each fails with its own message. refreshLicense removed with the modal; licence code untouched (Rules 18, 23).
