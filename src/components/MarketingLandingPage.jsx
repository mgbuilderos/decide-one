...
const FAQ = [['What Makes Decide One A Priority Instrument?', 'It does one job: help you decide what deserves attention first. List the competing work, choose one of three time-tested frameworks, put the priorities in order, and give the first one time. You still do the work.'], ['What Is The Evidence Behind The Approach?', 'The design draws on research into attention residue and specific action planning. The three prioritization methods are established practices with different origins. The evidence informs the product; Decide One itself has not been independently evaluated.'], ['Where Is My Work Stored?', 'Your entries are saved in this browser on this device. Export a backup before clearing browser data. Passphrase-protected exports are available.'], ['Can I Move Between Devices?', 'Use export and import to transfer your work. Each browser keeps its own local data; there is no automatic cloud sync.'], ['Can I Try Decide One First?', 'Yes. The complete prioritization instrument is available in this launch preview. Payment checkout will be connected separately.']];
...
export default function MarketingLandingPage({
  ...
  return <div ref={root} className={`pm-landing ${light ? 'pm-light' : ''} ${paused ? 'pm-motion-paused' : ''}`}>
    ...
    <main>
      ...
      <section className="pm-privacy-section" id="privacy"><div className="pm-privacy-icon"><Lock size={30} strokeWidth={1.2} /></div><p className="pm-eyebrow">Personal Means Personal</p><h2>Your priorities.<br /><span>Kept on your device.</span></h2><p>Your work stays in this browser. Export a protected copy before clearing browser data,<br />or use the privacy shutter when you need to hide the page.</p><a className="pm-text-link" href="#faq">Understand How Your Work Is Stored <ArrowDown size={15} /></a></section>
      <section className="pm-section pm-pricing-section" id="access"><div className="pm-pricing-copy"><p className="pm-eyebrow">Launch Preview</p><h2>Use the complete<br /><span>instrument now.</span></h2><p>Everything needed to choose, order,<br />and time today&rsquo;s priorities is ready.</p><button className="pm-text-link" onClick={onLaunchJournal}>Start Prioritizing <ArrowUpRight size={17} /></button></div><div className="pm-pricing-card"><div className="pm-pricing-top"><Target size={22} strokeWidth={1.4} /><span>Decide One</span><span>Launch Preview</span></div><h3 className="pm-access-title">Three methods. One clear order.</h3><ul>{['Top 3, Ivy Lee, and Urgent/Important Matrix', 'Realistic timeboxes and planned versus actual time', 'Daily, weekly, monthly, and yearly perspectives', 'Device-local storage, protected exports, and print'].map(text => <li key={text}><Check size={15} />{text}</li>)}</ul><button className="pm-button" onClick={onLaunchJournal}>Open Decide One <ArrowRight size={16} /></button><p className="pm-pricing-disclosure">Payment checkout is intentionally not connected in this build.</p></div></section>
