...
const HIGHLIGHTS = [{
  ...
  label: "02 / Build Your Rhythm",
  title: <>Small steps.<br />A lasting rhythm.</>,
  description: 'Repeat the habits that support your work. Notice your progress over time, and use reflection to decide what to change.',
  type: 'ritual'
}, {
...
export default function MarketingLandingPage({
  ...
  useEffect(() => {
    ...
    document.title = 'Decide One — The Priority Instrument';
    return () => {
      observer.disconnect();
  ...
  return <div ref={root} className={`pm-landing ${light ? 'pm-light' : ''} ${paused ? 'pm-motion-paused' : ''}`}>
    ...
    <header className="pm-nav"><a className="pm-brand" href="#overview" aria-label="Decide One home"><span className="pm-brand-mark">D1</span> Decide One</a><nav aria-label="Main navigation"><a href="#design">The Instrument</a><a href="#craft">The Details</a><a href="#pricing">Own Decide One</a></nav><div className="pm-nav-actions"><button className="pm-theme-button" onClick={toggleLight} aria-label={light ? 'Use dark appearance' : 'Use light appearance'}>{light ? <Moon size={16} /> : <Sun size={16} />}</button><button className="pm-button pm-button-small" onClick={onLaunchJournal}>Open Decide One <ArrowRight size={14} /></button></div></header>
    <main>
      <section className="pm-hero" id="overview"><div className="pm-hero-copy"><p className="pm-eyebrow">The Priority Instrument</p><h1>Choose your priorities.<br /><span>Start with what matters.</span></h1><p className="pm-hero-description">Decide One is your personal priority instrument.<br />List your tasks. Choose a framework. Decide what comes first.</p><div className="pm-actions"><button className="pm-button" onClick={onLaunchJournal}>Find Your Priorities <ArrowRight size={16} /></button><a className="pm-text-link" href="#design">Explore The Instrument <ArrowDown size={15} /></a></div></div><div className="pm-hero-product"><div className="pm-stage-glow" /><div className="pm-hero-render"><img src="/renders/decideone-studio-v2.png" alt="Decide One open with crisp white priority pages, fine stacked paper edges, a black cover and woven label" width="1536" height="1024" fetchpriority="high" /></div><div className="pm-product-caption"><span><i /> Made For A More Intentional Day</span><button onClick={() => setPaused(!paused)} aria-label={paused ? 'Play all animations' : 'Pause all animations'}>{paused ? <Play size={14} /> : <Pause size={14} />}</button><a href="#perspectives">Inspect In 3D <ArrowDown size={13} /></a></div></div></section>
      <div className="pm-belief-strip"><span>Your Priorities. Your Work.</span><div><span>Research-Informed Design</span><i /><span>Your Work, On Your Device</span><i /><span>No Subscription</span></div></div>
      ...
      <section className="pm-section pm-science-section" id="approach">
        <div className="pm-section-heading"><div><p className="pm-eyebrow">Designed Around How You Work</p><h2>Better habits.<br /><span>Grounded in research.</span></h2></div><p>Make fewer decisions about the tool.<br />Save your attention for the work.</p></div>
