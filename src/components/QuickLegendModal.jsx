...
export default function QuickLegendModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 dark:bg-black/70  flex items-center justify-center p-4 animate-in fade-in duration-200 select-none">
      ...
      <div className="relative w-full max-w-2xl max-h-[88vh] bg-white dark:bg-[#151515] text-neutral-900 dark:text-neutral-100 rounded-2xl shadow-2xl border border-black/[0.12] dark:border-white/[0.15] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              {FRAMEWORKS.map((method, index) => (
                <article key={method.id} className="p-4 rounded-xl border border-black/[0.08] dark:border-white/[0.08] bg-black/[0.015] dark:bg-white/[0.02]">
                  <div className="flex items-baseline justify-between gap-4">
                    ...
                    <span className="text-[11px] text-neutral-500 tabular-nums">0{index + 1}</span>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {SHORTCUTS.map(([key, description]) => (
                <div key={key} className="min-h-12 px-3 rounded-xl border border-black/[0.06] dark:border-white/[0.08] bg-black/[0.015] dark:bg-white/[0.02] flex items-center justify-between gap-4">
                  ...
                  <kbd className="px-2 py-1 rounded-md border border-black/15 dark:border-white/20 text-[11px] font-bold bg-white dark:bg-[#1b1b1b] shadow-2xs">{key}</kbd>
                </div>
              ))}
            </div>
          )}
