---
title: Why to-do lists stop working
description: A plain list runs in arrival order, the one ordering known to be bad. The problem is not discipline — a list records decisions instead of making it.
slug: why-to-do-lists-stop-working
intent: why to-do lists do not work
...
1. **Write everything down first.** Keep the full list. It is genuinely useful as a record, and you cannot choose between things you have not seen. It is just not a plan.
2. **Take a small number from it for today.** Three is a practical figure. The constraint is what makes it a decision rather than an intention.
3. **Give each one a duration.** A line without a time is a wish. The estimate does not need to be accurate; it needs to exist, so that afterwards you can compare planned against actual.
4. **Leave the rest visible and unchosen.** Not deleted. In scheduling terms this is **aging** — work that cannot run now moves up for next time. Nothing is lost by not being chosen today.

That is the whole of [the Top 3 method](/methods/top-3-method/), and it is what the long list becomes useful for: the pool you choose from, rather than the plan itself.
...
- [How to prioritise your tasks](/guides/how-to-prioritise-tasks/) — the three problems and which method fits each
- [The Top 3 method](/methods/top-3-method/) — the limit and the ordering rule in full
